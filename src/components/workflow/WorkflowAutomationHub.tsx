      ...
      <section className="rounded-2xl border border-[#222a3d] bg-[#0b1326] p-4 sm:p-5 space-y-4">
        ...
        <div className="grid gap-3 xl:grid-cols-[1.25fr_1fr_1fr]">
          ...
          <div className="rounded-xl border border-[#2b3851] bg-[#101a30] p-3.5">
            ...
            {assignmentMode === 'manual' && currentUserRole === 'executive' && <div className="mt-3 rounded-lg border border-[#3b82f6]/40 bg-[#0b1326] p-3"><label htmlFor="manual-assignee" className="text-[10px] font-mono uppercase text-[#86948a]">Founder-controlled assignee</label><select id="manual-assignee" value={selectedAssignee} onChange={(event) => setSelectedAssignee(event.target.value)} className="mt-1.5 w-full rounded-lg border border-[#2b3851] bg-[#131b2e] px-2.5 py-2 text-[11px] text-white"><option value="">Choose an available employee</option><option value="Maya Chen">Maya Chen · Shop drawings · 40% capacity</option><option value="Omar Rahman">Omar Rahman · Concrete / CSI 03 · 25% capacity</option><option value="Priya Shah">Priya Shah · Quantity takeoff · 60% capacity</option></select><button type="button" disabled={!selectedAssignee} onClick={() => setAssignmentNotice(`Project manually assigned to ${selectedAssignee}. Team lead notification queued.`)} className="mt-2 w-full rounded-lg bg-[#3b82f6] px-3 py-2 text-[10px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Confirm founder assignment</button></div>}

...
export const WorkflowAutomationHub: React.FC = () => {
  ...
  const [assignedProjects, setAssignedProjects] = useState<Array<{ id: string; title: string; route: 'smart' | 'manual' | 'outsourced'; assignee: string; status: string }>>([]);
      ...
      <section className="rounded-2xl border border-[#222a3d] bg-[#0b1326] p-4 sm:p-5 space-y-4">
        ...
        <div className="grid gap-3 xl:grid-cols-[1.25fr_1fr_1fr]">
          ...
          <div className="rounded-xl border border-[#2b3851] bg-[#101a30] p-3.5">
            ...
            {assignmentMode === 'outsourced' && <div className="mt-3 rounded-lg border border-[#e0b44a]/40 bg-[#0b1326] p-3"><label htmlFor="vendor-name" className="text-[10px] font-mono uppercase text-[#86948a]">Approved outsourcing vendor</label><input id="vendor-name" defaultValue="External delivery partner" className="mt-1.5 w-full rounded-lg border border-[#2b3851] bg-[#131b2e] px-2.5 py-2 text-[11px] text-white" /><button type="button" onClick={() => { setAssignedProjects((items) => [{ id: `PRJ-${items.length + 101}`, title: quotes.find((quote) => quote.status === 'paid_and_activated')?.title || 'New quoted project', route: 'outsourced', assignee: 'External delivery partner', status: 'Outsourced · awaiting vendor acceptance' }, ...items]); setAssignmentNotice('Outsourced project added to Active Projects and vendor approval queue.'); }} className="mt-2 w-full rounded-lg bg-[#e0b44a] px-3 py-2 text-[10px] font-bold text-[#07101f]">Approve and add to active projects</button></div>}
            ...
            <p className="mt-2 text-[10px] font-mono text-[#4edea3]">Route: {assignmentMode === 'smart' ? 'capacity + CSI match' : assignmentMode === 'manual' ? selectedAssignee ? `founder selection · ${selectedAssignee}` : 'founder-controlled selection' : 'vendor approval queue'}</p>{assignmentNotice && <p className="mt-1 text-[10px] text-[#bbcabf]">{assignmentNotice}</p>}
            {assignedProjects.length > 0 && <div className="mt-3 border-t border-[#222a3d] pt-3"><p className="text-[10px] font-mono uppercase text-[#86948a]">Active project list</p><div className="mt-2 space-y-2">{assignedProjects.map((project) => <div key={project.id} className="rounded-lg bg-[#0b1326] p-2 text-[10px]"><div className="flex justify-between gap-2"><span className="font-semibold text-white">{project.title}</span><span className="text-[#4edea3]">{project.route}</span></div><p className="mt-1 text-[#bbcabf]">{project.assignee} · {project.status}</p></div>)}</div></div>}
          </div>
          <div className="rounded-xl border border-[#2b3851] bg-[#101a30] p-3.5">
